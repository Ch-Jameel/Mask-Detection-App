        ...
        with open(label_output, 'w') as file:
            for obj in root.iter('object'):
                ...
                if class_name in object_classes:
                    ...
                    print(f"Processed {xml_file}: {class_idx} {' '.join(map(str, yolo_bbox))}")
                else:
                    print(f"Class '{class_name}' not in the defined classes.")
...
def prepare_image_data(image_files, img_dir, annot_dir, dest_dir, subset, object_classes):
    for image_file in image_files:
        # Copy images
        dest_image_path = os.path.join(dest_dir, f'Yolo_data/{subset}/images', image_file)
